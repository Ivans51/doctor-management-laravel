...
@section('content')
    <section class="max-w-3xl mx-auto mt-10 px-4">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold text-gray-800">Edit Medical Specialty</h1>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="w-full md:col-span-2">
                        ...
                        <input
                            ...
                            type="text" name="name" id="name" value="{{ $medical->name }}" required>
                    </div>
                    <div class="w-full md:col-span-2">
                        ...
                        <textarea class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
                            name="description" id="description" rows="4">{{ $medical->description }}</textarea>
                    </div>
                    <div class="w-full">
                        <label for="price" class="block text-sm font-medium text-gray-700 mb-1">Price</label>
                        <input
                            class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
                            type="text" name="price" id="price" value="{{ $medical->price }}" required>
                    </div>
                    <div class="w-full">
                        ...
                        <input
                            ...
                            type="tel" name="currency" id="currency" readonly value="{{ $medical->currency }}">
                    </div>
                </div>
                ...
                <div class="w-full mt-6">
                    ...
                    <select name="status" id="status"
                        ...
                        <option value="{{ Constants::$ACTIVE }}"
                            {{ $medical->status == Constants::$ACTIVE ? 'selected' : '' }}>Active</option>
                        <option value="{{ Constants::$INACTIVE }}"
                            {{ $medical->status == Constants::$INACTIVE ? 'selected' : '' }}>Inactive</option>
                    </select>
                </div>
