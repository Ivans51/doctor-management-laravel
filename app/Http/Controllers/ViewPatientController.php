...
class ViewPatientController extends Controller
{
     ...
     */
    public function getMonitoringForm(
        Request $request,
        ...
        try {
            ...
            $patient = Patient::query()
                ->with(["user", "doctorPatient", "doctorPatient.doctor"])
                ->where("id", $patientId)
                ->firstOrFail();
            ...
            return view(
                "pages/patient/checkout/monitoring-form",
                compact(["patient", "medicalSpecialties"]),
            );
        } catch (\Exception $e) {
            return redirect()
    ...
    public function getCheckoutForm(
        $encrypted_id,
        Request $request,
    ): View|Application|Factory|\Illuminate\Contracts\Foundation\Application|RedirectResponse {
        try {
            ...
            $appointment = Appointment::query()
                ->with(["patient", "schedule", "medicalSpecialty", "payment"])
                ->where("id", $appointmentId)
                ->orderBy("created_at", "desc")
                ->firstOrFail();

            return view(
                "pages/patient/checkout/checkout-form",
                compact("appointment"),
            );
        } catch (\Exception $e) {
            return redirect()
